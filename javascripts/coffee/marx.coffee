...
window.Marx = (options) ->
  ...
  xhr.onreadystatechange = =>
    ...
    if xhr.readyState is DONE
      if xhr.status is OK
        console.log @
        @marx_json = JSON.parse(xhr.responseText)
        @initialize(options)
...
Marx.prototype.initialize = (options) ->
...
Marx.prototype.extend = (defaults, options) ->
  extended = {}
